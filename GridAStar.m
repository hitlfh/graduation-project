function costmap = GridAStar(obstlist,goal,gres)
    [minx,miny,obmap] = CalcObstMap(obstlist,gres);  %gres为栅格的边长，设置过小会显著增加计算时间
    col = goal(1);
    row = goal(2);
    col = ceil((col-minx)/gres);
    row = ceil((row-miny)/gres);
    goal = [row,col];      %目标点的行列坐标
    costmap = 0*obmap; 
    dim = size(obmap);     
    for i = 1:dim(1)
        for j = 1:dim(2)
...
function [minx,miny,obmap] = CalcObstMap(obstlist,gres)
    ...
    for i = 1:ywidth
        for j = 1:xwidth
            ix = minx+(j-1/2)*gres;
            iy = miny+(i-1/2)*gres;
            [~,D] = knnsearch(obstlist,[ix,iy]);%k近邻算法实现计算点(ix, iy)距离obstlist中点最近的一个点的距离
            if D < 0.5
                obmap(i,j) = 1;
            end
        end
