function [x,y,th,D,delta] = HybridAStar(Start,End,Vehicle,Configure)  
    veh = Vehicle;
    cfg = Configure;
    mres = cfg.MOTION_RESOLUTION; % motion resolution 
    ...
    while ~isempty(Open)
        ...
        [Open,Close] = Update(wknode,Open,Close,veh,cfg); % 若该节点扩展出的RS曲线与障碍物发生碰撞，采用传统的扩展方式扩展
    end
end
...
function [x,y,th,D,delta] = getFinalPath(path,Close,veh,cfg)
    ...
    Close(end) = [];
    nodes = [wknode];  %nodes列表从后往前存储路径中的节点
...
function [wknode,nodes] = PopNode(nodes,cfg)  %这里的nodes就是Open集合
    mincost = inf;
    minidx = 1;
    gres = cfg.XY_GRID_RESOLUTION;
    ...
    wknode = nodes(minidx);
    nodes(minidx) = [];  %最后删除OPEN集合中的wknode
end
...
function [isok,path] = AnalysticExpantion(Start,End,Vehicle,Configure)
    ...
    % 终点位姿小于期望阈值也舍弃
    if (wrapToPi(pth) - End(3)) > deg2rad(5)
        isok = false;
    end
end
